# -*- coding: UTF-8 -*-
import sys,os,time
sys.path.append('/Users/cloudin1/PycharmProjects/untitled3/')
...
logfile = os.path.join('/Users/cloudin1/PycharmProjects/untitled3/aqi.log')
...
class mysql2pd(object):
    ...
    def write2mysql(self,dataframe,table):
        ...
        try:
            ...
            dataframe.to_sql(name=table, con=engine, if_exists='append', index=False,chunksize=1000, index_label=False)
            res=True
        except Exception as e:
            print(e)
...
def tecent(url,conn):
    ...
    res = res.text  # 需要转码，否则会有问题
    soup = BeautifulSoup(res, 'html.parser')  # 使用bs4模块，对响应的链接源代码进行html解析
    update_time=soup.find('div',{'class':'desc'}).text.split('数据更新时间: ')[1].split(' (每小时实时更新)')[0]
    data=soup.find_all('tr')
    res=[[c.text.strip() for c in r.find_all('td')] for r in data[1:]]
    res=pd.DataFrame(res,columns=['监测点','AQI指数','空气质量状况','PM2.5','PM10','Co','No2','So2','O3'])
    res['updata_time']=update_time
    conn.dopost("delete from aqi where updata_time='"+update_time+"'")
    conn.write2mysql(res,'aqi')
